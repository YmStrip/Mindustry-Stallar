		...
		Events.on(EventType.ContentInitEvent.class, e -> {
			...
			ST_TECH.createTechNodeRoot(ST_RESOURCE.SUPER_DRILL)
				...
				.add(ST_RESOURCE.SHALLOW_MINER, t -> t
					.add(ST_RESOURCE.SPECIAL_MINER, t1 -> t1
						.add(ST_RESOURCE.DEEP_MINER)
					)
				)
				.add(ST_RESOURCE.CONCENTRATOR);
			ST_TECH.createTechNodeRoot(ST_RESOURCE.SUPER_SOLID_PUMP)
				.require(Blocks.waterExtractor)
				.children(ST_RESOURCE.ATMOSPHERE_EXTRACTOR)
				.children(ST_RESOURCE.CORE_EXTRACTOR)
				.children(ST_RESOURCE.OIL_EXTRACTOR);
		});
	}
}
